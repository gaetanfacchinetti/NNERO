...
class Regressor(NeuralNetwork):
    ...
    def test(self, database, **kwargs):
        ...
        for i in range(0, database.metadata.ntest_valid):
            tau_pred = optical_depth(database.metadata.redshifts, prediction[i, :], ombh2[i], omch2[i] + ombh2[i], h[i])
            tau_true = optical_depth(database.metadata.redshifts, database.y_test_valid[i, :], ombh2[i], omch2[i] + ombh2[i], h[i])
